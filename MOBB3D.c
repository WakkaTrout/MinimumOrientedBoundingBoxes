...
// Brute Forces the 3D MOBB by considering all possible orientations (and a bunch extra), but breaks early in certain cases
// Runs in time O(n^7)
void MOBB3D_Brute_Force_Early_out(struct point3D *new_vals, size_t num_points, struct MOBB3D *output)
{
    double mobbVolume = DOUBLE_INFINITY;
    // Loop through triples of all pairs of points (brute force doesn't consider anything special about the edges)
    for ( size_t i = 0; i < num_points; ++i )
